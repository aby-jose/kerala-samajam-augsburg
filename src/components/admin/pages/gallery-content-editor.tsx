...
function HeadingFields({
  ...
  register: UseFormRegister<GalleryContentT>;
  errors: FieldErrors<GalleryContentT>;
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      ...
      <Field
        label="Lead"
        error={errors?.content?.[section]?.lead?.message}
...
export function GalleryContentEditor({ initialData }: { initialData: GalleryContentT }) {
  ...
  // you a uuid and GALLERY_SECTION_META[uuid] is undefined. Take the values
  // from watch() and use `field.id` only as the React key, where a stable
  // generated id is exactly what you want across a move().
  const layout = watch("layout");

  const onSubmit = async (data: GalleryContentT) => {
    try {
      await savePageContent("gallery", data);
      reset(data);
      success("Gallery page saved");
    } catch (e) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <PageHeader
        title="Gallery Page"
        description="Edit the copy and the order of the sections on the public Gallery page."
      >
      ...
      {fields.map((field, index) => {
        ...
        const meta = GALLERY_SECTION_META[entry.id as GallerySectionId];
