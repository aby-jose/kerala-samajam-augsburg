import { EVENTS_SECTION_IDS, type EventsSectionId } from "./events";
 ...
 * in app/(public)/events/events-client.tsx.
 */
export type { SurfaceMode };

export const EVENTS_SECTION_META: Record<EventsSectionId, SectionMeta> = {
  hero: {
    label: "Page header",
    description: "The eyebrow, title, lead paragraph and the spotlight of upcoming events at the top of the page.",
    surfaceMode: "rotate",
    ...
    // against the navbar with no clearance — the same reason About's and
    // Contact's heroes are pinned.
    movable: false,
  ...
  calendar: {
    label: "Rest of the calendar",
    description: "Everything not in the spotlight, listed as cards in date order. Hidden entirely when the spotlight already covers every event.",
    surfaceMode: "rotate",
    movable: true,
  },
  membersBand: {
    label: "Members first",
    description: "The dark band closing the page, inviting people to join for early access to new dates.",
    surfaceMode: "deep",
    movable: true,
  },
  whatsappCta: {
...
export const DEFAULT_EVENTS_SECTION_ORDER: readonly EventsSectionId[] = EVENTS_SECTION_IDS;
