...
/**
 * 1. Page header — pinned to the top (see GALLERY_SECTION_META), so its
...
function GalleryHeroSection({
  content = DEFAULT_GALLERY.content.hero,
  surface = "bg-surface-1",
  stats,
  onFindPhotos,
  onAddPhotos,
}: {
  content?: GalleryContentT["content"]["hero"];
...
/**
 * 2. The albums. Tinted (bg-surface-2) at the default order — position-
 * derived, see lib/page-layout.ts.
 */
function GalleryAlbumsSection({
  content = DEFAULT_GALLERY.content.albums,
  surface = "bg-surface-2",
  bordered = true,
  albums,
  onAddPhotos,
}: {
  content?: GalleryContentT["content"]["albums"];
  albums: Album[];
  onAddPhotos: () => void;
} & SectionSurfaceProps) {
  const rise = useRiseVariants();
  ...
  return (
    <section
      id="albums-grid"
      className={cn(surface, "py-24 md:py-32", bordered && "border-y border-border")}
    >
      <Container className="max-w-7xl">
        <div className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
          <div className="max-w-xl">
            ...
            {/* Empty by default — see lib/page-content/gallery.ts. Rendered
                only when an admin fills it in, so the layout stays exactly
                as it is today until then. */}
            {content.lead && (
              <SectionLead className="mt-4">{content.lead}</SectionLead>
            )}
          </div>
...
/**
 * 3. Contribute — closing band. Same ambient treatment as the closing bands
 * on /about, /contact and /events, so the pages end on one note.
 */
function GalleryContributeSection({
  content = DEFAULT_GALLERY.content.contribute,
  surface = "bg-surface-deep",
  onUploadPhotos,
}: {
  content?: GalleryContentT["content"]["contribute"];
  onUploadPhotos: () => void;
} & SectionSurfaceProps) {
  const rise = useRiseVariants();
...
export default function GalleryLandingClient({
  ...
  const sections = resolveSections(GALLERY_SECTION_META, content.layout);
  ...
  return (
    <>
      {sections.map(({ id, surface, bordered }) => {
        if (id === "hero") {
          return (
            <GalleryHeroSection
              ...
              surface={surface}
              stats={stats}
              onFindPhotos={requireSession(() => setIsFaceSearchOpen(true))}
        ...
        if (id === "albums") {
          return (
            <GalleryAlbumsSection
              key={id}
              content={content.content.albums}
              surface={surface}
              bordered={bordered}
              albums={albums}
              onAddPhotos={requireSession(() => setIsContributionModalOpen(true))}
        ...
        return (
          <GalleryContributeSection
            key={id}
            content={content.content.contribute}
            surface={surface}
            onUploadPhotos={requireSession(() => setIsContributionModalOpen(true))}
          />
        );
      })}
