...
**Files:**
- Modify: `src/lib/event-actions.ts` (14 sites), `src/lib/ticket-actions.ts` (1 site)
...
In `src/lib/ticket-actions.ts`, `sendEventTicket` becomes `await requirePermission("registrations.ticket");`.
...
```bash
git add src/lib/event-actions.ts src/lib/ticket-actions.ts
...
**Files:**
- Modify: `src/lib/payment-actions.ts` (3 sites), `src/lib/membership-actions.ts` (13 sites), `src/lib/invoice-actions.ts`
...
- [ ] **Step 3: Convert invoice-actions.ts**

Both `sendMembershipPaymentRequest` and `sendSubscriptionReceipt` take `await requirePermission("payments.invoice");`. If either currently has no guard, add one at the top of the function.
...
```bash
git add src/lib/payment-actions.ts src/lib/membership-actions.ts src/lib/invoice-actions.ts
