...
- [ ] **Step 3: Delete the old guards**

In `src/lib/guards.ts`, remove `requireAdmin` and `requireAdminPage` entirely. Keep `getAdminUser` — `getStaffContext` depends on it.

The pages still calling `requireAdminPage` will now fail to compile. That is intentional and Task 10 fixes them; do not proceed to Step 5 until Task 10 is done if you need a green `tsc`.
...
- [ ] **Step 2: Open the invite route in middleware**
...
- [ ] **Step 3: Verify no page is left unguarded**
...
```bash
git add "src/app/admin/(dashboard)" src/proxy.ts
