...
| `scratch/seed-roles.ts` | Idempotent seed + admin backfill |
...
- Create: `src/lib/rbac/presets.ts`, `scratch/seed-roles.ts`, `tests/rbac/presets.test.ts`
 ...
 *
 * Run with: npx tsx scratch/seed-roles.ts
 */
...
Run: `npx tsx scratch/seed-roles.ts && npx tsx scratch/seed-roles.ts`
Expected: first run creates six roles and backfills; second run reports all six as `(exists, untouched)` and backfills 0

- [ ] **Step 9: Commit**

```bash
git add prisma/schema.prisma src/lib/rbac/presets.ts scratch/seed-roles.ts tests/rbac/presets.test.ts
...
2. **Run `npx tsx scratch/seed-roles.ts` against production.** Until this runs, every administrator holds the portal gate with no permissions and no admin page will render.
